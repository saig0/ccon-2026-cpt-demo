...
export function startWorker(): void {
  ...
  zbc.createWorker({
    ...
    taskHandler: async (job) => {
      ...
      console.log(
        `[Worker] Processing message for conversation ${conversationId} (user: ${userName}): "${currentMessage}"`
      );

      const agentMessage = generateAgentResponse(currentMessage ?? '');
      ...
      console.log(`[Worker] Agent responded to ${conversationId}: "${agentMessage}"`);

      return job.complete({ lastAgentResponse: agentMessage });
    },
  });
